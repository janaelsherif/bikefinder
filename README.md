...
| Next.js dev error `Cannot find module './vendor-chunks/@swc.js'`, **`Cannot find module './682.js'`** (or any missing numbered chunk under `.next/server`) | Stale dev cache / HMR chunk churn. Stop **all** `next dev` processes, run **`cd frontend && npm run dev:clean`**. Default **`npm run dev`** no longer forces Watchpack polling (fewer spurious rebuilds on macOS). Use **`npm run dev:poll`** only in Docker/VM/NFS where file events are unreliable. Avoid **`npm run dev:turbo`** with next-intl unless you must. Moving the repo to a path **without spaces** (e.g. out of `fl p2`) also helps tooling stability. |
